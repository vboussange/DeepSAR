...
if __name__ == "__main__":
    ...
    for hab in habitats:
        ...
        gdf_test = gdf[gdf.num_plots == 1]
        reg = results_fit_split[hab]["reg"]
        feature_scaler, target_scaler = results_fit_split[hab]["scalers"]
        X, y, feature_scaler, target_scaler = NNSAR_fit_simple.get_Xy_scaled(gdf_test, climate_predictors, feature_scaler=feature_scaler,  target_scaler=target_scaler)


        log_sr_pred, logc, z = calculate_sr_logc_z(reg, X, feature_scaler, target_scaler)
    ...
    sns.boxplot(df_c_z, x="habitat_id", y="logc", palette=COLOR_PALETTE, ax=axes[0], width=0.5, linewidth=0.1)
    sns.boxplot(df_c_z, x="habitat_id", y="z", palette=COLOR_PALETTE, ax=axes[1], width=0.5, linewidth=0.1)
