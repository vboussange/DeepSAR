...
def calculate_sr_logc_z(reg, X, feature_scaler, target_scaler):
    ...
    with torch.no_grad():
        ...
        logc = out[:,0]
        z = out[:, 1] * (out[:, 1] > 0)
        logc = target_scaler.scale_[0] * logc
        z = target_scaler.scale_[0] * z / feature_scaler.scale_[0]
        
        log_area = feature_scaler.scale_[0] * X["log_area"].flatten()
...
if __name__ == "__main__":
    result_path = Path(__file__).parent / Path("../../../scripts/NNSAR/NNSAR_fit_simple_max_scaling.pkl")
