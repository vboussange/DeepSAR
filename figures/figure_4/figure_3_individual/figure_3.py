...
def plot_SAR_dSAR(ax_SAR, ax_dSAR, model, results_fit_split, gdf):
    ...
    for plottype in mydict:
        ...
        # print(ys)
        for ax, ys, label in zip((ax_SAR, ax_dSAR), [ys_SAR, ys_dSAR], ("SR", "dlogSR/dlogA")):
...
if __name__ == "__main__":
    ...
    hab = "all"
    checkpoint_path = f"../../../scripts/MLP3/results/MLP_fit_torch_{hab}_dSRdA_weight_1e-01_seed_4/checkpoint_best.pth"
    model, results_fit_split = load_model_checkpoint(checkpoint_path, "cpu")
    if "seed" not in results_fit_split.keys():
        results_fit_split["seed"] = 42
    gdf = preprocess_gdf_hab(dataset.gdf, hab, results_fit_split["seed"])
    ...
    # # SAR
    plot_SAR_dSAR(ax1, ax2, model, results_fit_split, gdf)
    # ax.set_ylim(1, 1e2)
    ax1.set_yscale("log")
    ax1.set_xscale("log")
    ax1.legend()
