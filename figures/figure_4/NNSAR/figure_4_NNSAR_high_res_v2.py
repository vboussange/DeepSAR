"""
in v2 we use augmented dataset with plot:megaplot ratio 1:1
"""
...
import NNSAR_project_coarse_nruns_plot_megaplot as NNSAR_project
...
with open(NNSAR_project.PATH_RESULT, 'rb') as file:
...
fig_mean_run, axs_mean_run = plt.subplots(1, 3, figsize=(9, 4))
fig_std_run, axs_std_run = plt.subplots(1, 3,figsize=(9, 4))
...
for j, quantity in enumerate(["sr", "logc", "z"]):
    ...
    titles = plot_metadata[quantity]
    cmap = "OrRd" if quantity == "z" else "PuBu" if quantity == "logc" else "BuGn"
    plot_raster(mean_rast, titles, axs_mean_run[j], cmap)
    plot_raster(std_rast, titles, axs_std_run[j], cmap)
    for ax in [axs_mean_run[j], axs_std_run[j]]:
        ax.set_xticklabels([])
        ax.set_yticklabels([])
        ax.set_xticks([])
        ax.set_yticks([])
    if j == 0:
        axs_mean_run[j].set_ylabel(f"Resolution: {ncells}km$^2$")
        axs_std_run[j].set_ylabel(f"Resolution: {ncells}km$^2$")

fig_mean_run.tight_layout()
fig_std_run.tight_layout()
fig_mean_run.savefig(f"mean_run_c_z_sr_coarsening_hig_res_{ncells}.png", transparent=True, dpi=300)
fig_mean_run.savefig(f"std_run_c_z_sr_coarsening_hig_res_{ncells}.png", transparent=True, dpi=300)
