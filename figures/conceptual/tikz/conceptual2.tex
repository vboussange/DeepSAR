...
\begin{tikzpicture}[
    ...
    \node (lc) at(-1, 3.5) {\includegraphics[width=6.2cm]{landcover_real.png}};

    \node[mynodeorange, minimum height=13.5cm, minimum width=7.6cm] (box) at (-1.8, -6.8) {};
    \node[] (box_text) at (-2., -0.5) {Raw environmental data};
    \node (cl1) at (-1.8, -4.0) {\includegraphics[width=7.8cm]{chelsa_real_bio1.png}};
    \node (cl2) at (-1, -7.25) {$\vdots$};
    \node (cl3) at (-1.8, -10.5) {\includegraphics[width=7.8cm]{chelsa_real_bio4.png}};

    \node (avp) at (7, 6.0) {Augmented vegetation plot $j$};
    \node (vp) [below=0.1cm of avp] {Vegetation plot $i$};
    \node (hm) [below=0.1cm of vp] {Habitat mask};
    \node (sb) [below=0.1cm of hm] {$100 \times 100$ km$^2$ Spatial block};
  ...
  ([xshift=-2.2cm, yshift=-3.2cm]lc.north) -- ([xshift=-2.2cm,yshift=0.4cm]lc.south) node[midway,xshift=-4em,rotate=90]{\normalsize 100km};
    ...
    \node[mynode, draw=mygrey!30, fill=mygrey!10, align=center] (model) [right=6cm of dots, yshift=3cm] {Supervised \\learning\\[1em] \Huge{$\mathcal{M}(A_j, \mu_j, \sigma_j) = \hat{SR}_j$}};
    \node[mynodeblue] (mu) [left=0.1cm of model, yshift=0.1cm] {$\mu_j$};
    \node[mynodered] (sigma) [below=0.1cm of mu] {$\sigma_j$};
    \node[mynode] (y) [above=0.1cm of model] {$y_j$};
    ...
    \draw[arrow] (SR.east) -| (y.north);
    \draw[arrow] (physics.north) -| (model.south);


    \draw[arrow] (Area.east) -| ([xshift=-1cm, yshift=0.8cm]model.west) |- ([yshift=0.8cm]model.west);
    \draw[arrow] (mean1.east) -| ([xshift=-1cm, yshift=0.3cm]sigma.west) |- (mu.west);
    \draw[arrow] (std1.east) -| ([xshift=-1cm,yshift=-0.1cm]sigma.west) |- (sigma.west);
    \draw[arrow] (mean2.east) -| ([xshift=-1cm]sigma.west) |-  (mu.west);
    \draw[arrow] (std2.east) -| ([xshift=-1cm, yshift=-1cm]sigma.west) |- (sigma.west);
