...
def estimate_sr(plots, species_dict):
    ...
    if f2 == 0:
        chao1 = S_obs + (f1 * (f1 - 1)) / 2  # bias-corrected if no doubletons
        var_chao1 = (f1 * (f1 - 1)) / 2 + ((f1 * (2 * f1 - 1) ** 2) / 4)
    else:
        chao1 = S_obs + (f1 ** 2) / (2 * f2)
        var_chao1 = f2 * ((f1 / f2) ** 4) / 4 + ((f1 ** 3) / (2 * f2 ** 2))    
    
    return S_obs, chao1, var_chao1
...
gift_dataset["chao1"] = 0
gift_dataset["var_chao1"] = 0
...
for idx, row in tqdm(gift_dataset.iterrows(), total=gift_dataset.shape[0]):
    ...
    if len(df_box) == 0:
        gift_dataset.at[idx, "chao1"] = np.nan
        gift_dataset.at[idx, "var_chao1"] = np.nan
        gift_dataset.at[idx, "eva_observed_sr"] = np.nan
        continue
    eva_observed_sr, chao1, var_chao1 = estimate_sr(df_box, eva_species_dict)
    gift_dataset.at[idx, "chao1"] = chao1
    gift_dataset.at[idx, "var_chao1"] = var_chao1
    gift_dataset.at[idx, "eva_observed_sr"] = eva_observed_sr


    print(
        f"Index: {idx} | Chao1: {chao1:.2f} | Var(Chao1): {var_chao1:.2f} | "
...
axes[0].scatter(gift_dataset["gift_observed_sr"], gift_dataset["chao1"], alpha=0.7)
max_val = np.nanmax([gift_dataset["gift_observed_sr"].max(), gift_dataset["chao1"].max()])
axes[0].plot([0, max_val], [0, max_val], 'r--', label='1:1 line')
axes[0].set_xlabel("GIFT observed SR")
axes[0].set_ylabel("Chao1 estimated SR")
...
mse_0 = root_mean_squared_error(mask0["gift_observed_sr"], mask0["chao1"])
axes[0].text(
...
mse_1 = root_mean_squared_error(mask1["gift_observed_sr"], mask1["eva_observed_sr"])
axes[1].text(
