...
def create_raster_sr(X_map, reg):
    y_map = reg.predict(X_map)
    Xy_map = X_map.copy()
    Xy_map["sr"] = np.exp(y_map)
    sr_raster = Xy_map[["sr"]].to_xarray()
    return sr_raster["sr"].sortby(["y", "x"])
...
if __name__ == "__main__":
    ...
    fig.tight_layout()
    fig.savefig("figure_4_EVA_Copernicus.png", dpi=300, transparent=True)