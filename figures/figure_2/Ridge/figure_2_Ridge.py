...
sys.path.append(str(Path(__file__).parent / Path("../../scripts/GLM/")))
import Ridge_fit_pred_comb_SBCV as Ridge_fit
...
def evaluate_model_all_residuals(gdf, predictors):
    ...
    X, y, feature_scaler, target_scaler = Ridge_fit.get_Xy_scaled(gdf, predictors)

    print("Evaluation of model with area")
    results = {}
    reg = NeuralNetRegressor(module=SimpleNNBatchNorm,
                            module__input_dim=1,
                            **Ridge_fit.CONFIG["torch_params"])
...
if __name__ == "__main__":
    ...
    with open(Ridge_fit.PATH_RESULTS, 'rb') as file:
