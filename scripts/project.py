...
DEFAULT_RUN_DIR = (
    ...
    / "results/benchmark/artifacts/MuScaRi_ClimateDEM/dae0789a3c87"
)
RUN_DIR = Path(os.environ.get("MUSCARI_PROJECT_RUN_DIR", DEFAULT_RUN_DIR))
EXPORT_DIR = RUN_DIR / "ensemble_pretrained"
MODEL_NAME = os.environ.get("MUSCARI_PROJECT_MODEL_NAME", "MuScaRi_ClimateDEM")
...
def batch_predict(model, env_dataset, lc_dataset, res, batch_size=4096):
    """Calculate asymptotic species richness at a specified resolution."""
    mean_SR_list = []
...
if __name__ == "__main__":
    ...
    for res in RESOLUTIONS_M:
        print(f"Calculating SR for resolution: {res}m")
        features, SR = batch_predict(model, env_dataset, lc_dataset, res)
