...
def estimate_sr_mm(x, y):
    if len(x) < 2:
        return np.nan, np.nan
    # Initial guess: a=max(y)*1.5, b=median(x)
    try:
        popt, pcov = curve_fit(michaelis_menten, x, y, p0=[max(y)*1.5, np.median(x)], maxfev=10000)
        a = popt[0]
        b = popt[1]
        return a, b
...
if len(df_test) > 0:
    ...
    a, b = estimate_sr_mm(x, y)

    # Predict species richness at GIFT polygon area
    predicted_sr = michaelis_menten(gift_polygon_area, a, b)
    ...
    y_fit = michaelis_menten(x_fit, a, b)
    ...
    ax.set_title(f'Rarefaction curve with Michaelis-Menten fit, {country}')
    ax.legend()

    # Add text with fit information
    ax.text(0.05, 0.8, f"Michaelis-Menten parameters: a={a:.1f}, b={b:.1f}\n"
