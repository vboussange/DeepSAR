...
def logistic_4p(x, a, b, c, d):
    """4-parameter logistic function: f(x) = d + (a - d) / (1 + (x / c) ^ b)
    a: upper asymptote
...
def estimate_sr_logistic(x, y):
    ...
    # Initial guess: a=max(y)*1.2, b=1, c=median(x), d=min(y)
    try:
        popt, pcov = curve_fit(
            logistic_4p, x, y, 
            p0=[max(y)*1.2, 1, np.median(x), min(y)], 
            maxfev=10000
        )
        a, b, c, d = popt
        return a, b, c, d
    except Exception as e:
        logging.warning(f"Logistic curve fit failed: {e}")
        return np.nan, np.nan, np.nan, np.nan
...
if len(df_test) > 0:
    ...
    a, b, c, d = estimate_sr_logistic(np.log(x), y)

    # Predict species richness at GIFT polygon area
    predicted_sr = logistic_4p(np.log(gift_polygon_area), a, b, c, d)
    ...
    y_fit = logistic_4p(np.log(x_fit), a, b, c, d)
    ...
    ax.set_title(f'Rarefaction curve with 4PL fit, {country}')
    ax.legend()

    # Add text with fit information
    ax.text(0.05, 0.8, f"4PL parameters: a={a:.1f}, b={b:.3f}, c={c:.1f}, d={d:.1f}\n"
...
ax.legend()
ax.set_title("GIFT vs predicted species richness from 4PL fit")
