...
def estimate_sr_log(x, y):
    if len(x) < 2:
        return np.nan, np.nan
    
    # Convert x to log space
    logx = np.log(x)
    
    try:
        # Fit the linear model in log space
        # Using linear regression since S = a + b*log(A) is linear in log-transformed data
        slope, intercept, r_value, p_value, std_err = linregress(logx, y)
        return intercept, slope  # return as (a, b) for the model a + b*log(x)
    except Exception as e:
        logging.warning(f"Linear fit failed: {e}")
        return np.nan, np.nan
...
if len(df_test) > 0:
    ...
    # Fit logarithmic model S = a + b*log(A)
    a, b = estimate_sr_log(x, y)

    # Predict species richness at GIFT polygon area
    predicted_sr = logarithmic_model(gift_polygon_area, a, b)
    ...
    x_fit = np.logspace(np.log10(min(x)), np.log10(max(x) * 10), 100)
    ...
    ax.set_title(f'Species-Area Curve with logarithmic fit, {country}')
    ax.legend()

    # Add text with fit information
    ax.text(0.05, 0.8, f"Logarithmic model parameters: a={a:.1f}, b={b:.1f}\n"
                        f"S = a + b*log(A)\n"
