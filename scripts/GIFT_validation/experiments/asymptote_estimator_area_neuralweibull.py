"""
Total species richness estimation using rarefaction curve fitted with NeuralWeibull model, without any environmental covariates.
"""
...
def estimate_sr_neural_weibull(x, y):
    ...
    model = NeuralNetRegressor(Neural4PWeibull,
                            ...
                            callbacks=[EarlyStopping(patience=50, threshold=1e-4)],
                        )

    model.fit(x.reshape(-1, 1), y)
    return model
    
# model = Neural4PWeibull(
#     n_features=0, 
#     layer_sizes=[], 
#     p0=np.array([3.584, 2397.9, -70.2, 12.1])
# )
# X = torch.tensor([1., 2., 3., 4., 5,]).reshape(-1, 1)
# model(X)
# model.predict_b_c_d_e(torch.tensor([]).reshape(1, 0))
...
if len(df_test) > 0:
    ...
    # area, sr = rarefaction_curve(df_test, eva_species_dict)
    # x = np.log(area)
    # x = x / x.max()  # Normalize x to [0, 1] for better fitting
    # y = sr / sr.max()  # Normalize y to [0, 1] for better fitting
    # Fit Weibull model
    x = np.log(x)
    
    model = estimate_sr_neural_weibull(x, y)
    b, c, d, e = model.module_.predict_b_c_d_e(torch.tensor([]).reshape(1, 0).to("cuda" if torch.cuda.is_available() else "cpu"))
    print(f"Fitted parameters: b={b.item()}, c={c.item()}, d={d.item()}, e={e.item()}")

    # Predict species richness at GIFT polygon area
    predicted_sr = c.item()  # This is the fitted asymptote
    ...
    x_fit = np.linspace(min(x), max(x) * 2, 100, dtype="float32")
    y_fit = model.predict((x_fit).reshape(-1, 1))
    ax.plot(x_fit, y_fit, 'r-')

    # Plot the GIFT prediction point
    ax.scatter(np.log(gift_polygon_area), predicted_sr, marker='*', s=200, color='green', 
                label=f"Predicted SR: {predicted_sr:.1f}")
    ax.scatter(np.log(gift_polygon_area), gift_observed_sr, marker='*', s=200, color='blue',
