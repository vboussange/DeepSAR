...
def process_partition(partition, block_plot_gdf, species_dict, climate_raster):
    ...
    boxes_gdf = generate_random_squares(block_plot_gdf, 
                                      min(len(block_plot_gdf), CONFIG["num_polygon_max"]), 
                                      CONFIG["area_range"])
    megaplot_data_partition = clip_EVA_SR(block_plot_gdf, species_dict, boxes_gdf)
...
def generate_megaplots(plot_gdf, species_dict, climate_raster):
    """
    ...
    for partition, block_plot_gdf in tqdm(plot_gdf.groupby("partition"), desc="Processing partitions", total=total, miniters=miniters):
        if len(block_plot_gdf) > 1:
            megaplot_data_hab_ar.append(process_partition(partition, block_plot_gdf, species_dict, climate_raster))
